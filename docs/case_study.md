...
_Real tickets. Real Linux systems. Real guardrails._
...
- The technician logs into a React workspace and sees their open tickets, sortable by date (default), priority and customer.
- They open a ticket and read the customer's report. The app loads the customer system information, including the SSH connection details, from the ERP.
- AI agents analyse the ticket together with the system context and propose how to proceed. The technician explicitly approves connecting to the customer VM.
- The AI proposes diagnostic and fix steps. The technician confirms, edits or rejects each one. A human confirmation is mandatory for every action the AI takes on the system, and the technician can step in or stop at any point.
- Every action the AI takes is logged automatically, and that running log becomes the basis for a precise activity description - not a generic summary written after the fact.
- Once the fix is validated, the technician submits the finished activity - root cause, actions taken, commands, validation and result - back to the ERP.

How you orchestrate this is up to you: a single planning agent with tools, or several specialised agents (for example problem_analyzer, customer_system_analyzer, problem_solver and activity_log_generator) - as long as the whole flow works and the technician stays in control. The customer VMs run Ubuntu Linux, but aim for an approach that is, in principle, OS-agnostic rather than tied to specific problems.
...
**Required activity fields:** summary, root_cause (the technical cause, not the symptom), actions_taken (diagnosis and fix steps in order), commands_summary (relevant commands or command classes, no secret output), and validation_result (concrete proof the customer benefit is restored).
